...
\begin{equation}
    \textbf{AN : } f_{step} = \frac{v_{max}}{\text{Résolution}} = \frac{50}{(5 / (200 \times 16))} = \frac{50}{0,0015625} \approx \boxed{32~000 \text{ Hz}}
\end{equation}
...
\begin{equation}
    \textbf{AN : } I_{\text{entree}} = \frac{V_{uP} - V_f}{R} = \frac{3,0 - 1,4}{270} \approx \boxed{5,9 \text{ mA}}
\end{equation}
...
\begin{equation}
    \textbf{AN : } I_{total\_RMS} \approx 0,707 \cdot n \cdot I_{max} \approx 0,707 \times 5 \times 2 = \boxed{7,07 \text{ A}}
\end{equation}
La puissance nominale à dissiper de l'alimentation SMPS (Switch-Mode Power Supply) sera donc :
\begin{equation}
    \textbf{AN : } P_{PSU} = V \times I \times K_{\text{sécu}} = 36 \times 7,07 \times 1,25 \approx \boxed{318 \text{ Watts}}
\end{equation}
...
\begin{equation}
    \textbf{AN : } C_{bulk} = 0,5 \times \frac{20 \cdot 10^{-6}}{0,1} = 100 \cdot 10^{-6} \text{ F} = \boxed{100 \mu \text{F}}
\end{equation}
...
\section{Problématiques CEM et Intégrité des Signaux}
La commutation rapide des courants (PWM à $20 \text{ kHz}$) dans les câbles des moteurs génère des champs électromagnétiques parasites susceptibles de corrompre les signaux de commande TTL de l'ESP32.

\subsection{Couplage et Blindage}
Afin de minimiser le couplage capacitif et inductif, nous avons adopté une stratégie de blindage rigoureuse : 
\begin{itemize}
    \item \textbf{Séparation des câbles :} Les lignes de puissance et de signal sont physiquement espacées d'au moins $10 \text{ cm}$.
    \item \textbf{Câbles blindés (Braided Shield) :} Le moteur de broche (inverter) utilise des câbles dont le blindage est relié à la terre protectrice ($PE$) d'un seul côté pour éviter les boucles de masse.
    \item \textbf{Ferrites de protection :} Des anneaux de ferrite ont été placés sur les lignes Step/Dir pour absorber les pics de haute fréquence.
\end{itemize}

\subsection{Bilan Thermique et Analyse du Flux d'Air}
Les cinq drivers DM556 et l'alimentation PSU dissipent une énergie calorifique non négligeable par effet Joule. Pour éviter le "Thermal Throttling" ou la destruction des composants, un ventilateur d'extraction a été calculé. 
La puissance totale dissipée estimée est $P_{diss} \approx 40 \text{ W}$. Pour maintenir une élévation de température $\Delta T < 15^\circ\text{C}$ à l'intérieur du coffret, le débit d'air nécessaire $Q$ est :
\begin{equation}
    \textbf{AN : } Q = \frac{3 \times P_{diss}}{\Delta T} \approx \frac{3 \times 40}{15} = \boxed{8 \text{ m}^3\text{/h}}
\end{equation}
Un ventilateur de $80 \times 80 \text{ mm}$ (débit $40 \text{ m}^3\text{/h}$) a été installé, garantissant une marge de sécurité thermique confortable.

\section{Dimensionnement Électrique et Bilan de Puissance}

Une commande numérique 5 axes présente un cumul critique de consommateurs de courant (les cinq moteurs PàP fonctionnent quasi-simultanément). Le calcul rigoureux du bilan énergétique est impératif pour dimensionner l'Alimentation à Découpage (SMPS) principale et éviter une chute de tension (Brown-out) risquant de faire décrocher un moteur en plein usinage.

\subsection{Consommation théorique (Bilan d'intensité)}
Les composants actifs nécessitant une évaluation de puissance sont :
\begin{itemize}
    \item \textbf{Moteurs NEMA 17 (x5) :} Le courant nominal maximal par phase d'un `17HS4401` est $I_m = 1.7 \text{ A}$. Nous utilisons une configuration Bipolaire avec un réglage de V-Ref sur le driver limitant le courant efficace RMS à environ $1.4 \text{ A}$ pour limiter l'échauffement Joule. La consommation totale des moteurs bloqués (Holding Torque) est de : $P_{moteurs} = 5 \times 1.4 \text{ A} = 7.0 \text{ A}$.
    \item \textbf{Électronique de Supervision (ESP32 + Wi-Fi) :} L'activation de la radio RF de l'ESP32 requiert un courant en crête $I_{pic} = 500 \text{ mA}$ sur la ligne $3.3 \text{ V}$, soit une dissipation indirecte (via le convertisseur DC-DC Step-Down $36V \rightarrow 5V$) négligeable de $0.2 \text{ A}$ (coté 36V).
    \item \textbf{Pertes et ventilateurs :} Les composants passifs, les LEDs et les ventilateurs annexes tirent environ $0.5 \text{ A}$.
\end{itemize}

\subsection{Sélection de l'Alimentation Primaire (SMPS)}

L'intensité totale projetée au régime critique est :
\begin{equation}
    \textbf{AN : } I_{total\_RMS} = \sum I = 7,0 + 0,2 + 0,5 = \boxed{7,7 \text{ Ampères}}
\end{equation}

Pour l'alimentation des moteurs pas à pas, on opte pour une tension de $36 \text{ VDC}$ au lieu de $12 \text{ VDC}$ ou $24 \text{ VDC}$. L'augmentation de la tension diminue drastiquement le temps d'établissement du courant dans l'inductance (di/dt) des bobines, permettant au moteur de garder son couple résistant même à très haute vitesse angulaire.
Le besoin en puissance électrique brute au primaire est donc :
\begin{equation}
    \textbf{AN : } P_{calcul} \approx 36 \text{ V} \times 7,7 \text{ A} = \boxed{277 \text{ Watts}}
\end{equation}

Afin d'appliquer un coefficient de sécurité industriel ($\times 1.25$) qui empêchera l'alimentation de chauffer en fonctionnant à la limite de ses condensateurs, la \textbf{SMPS choisie est un modèle LRS-350-36 (36 V / 9.7 A / 350 W)}.
